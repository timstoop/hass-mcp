...
async def list_entities(
    ...
    # Ensure we return a list even if there's an error
...
async def search_entities_tool(query: str, limit: int = 20) -> dict[str, Any]:
    ...
    if not query or not query.strip():
        ...
        # Type narrowing: at this point entities must be a list
        if not isinstance(entities, list):
            return {"error": "Unexpected response format", "count": 0, "results": [], "domains": {}}

        # No query, but we'll return a structured result anyway
        domain_counts: dict[str, int] = {}
        simplified_entities = []

        # At this point we know entities is a list due to earlier error handling
        for entity in entities:
            domain = entity["entity_id"].split(".")[0]
...
async def search_entities_resource_with_limit(query: str, limit: str) -> str:
    ...
    # Group entities by domain for better organization
    domains: dict[str, list[dict[str, Any]]] = {}
    # Type narrowing: at this point we know entities is a list due to earlier error handling
    entities_list = cast("list[dict[str, Any]]", entities)
    ...
    simplified_entities = []
    # Type narrowing: at this point entities must be a list due to earlier error handling
    entities_list = cast("list[dict[str, Any]]", entities)
    for entity in entities_list:
        simplified_entity = {
            "entity_id": entity["entity_id"],
...
async def list_states_by_domain_resource(domain: str) -> str:
    ...
    # Type narrowing: at this point we know entities is a list due to earlier error handling
    entities_list = cast("list[dict[str, Any]]", entities)
    for entity in sorted(entities_list, key=lambda e: e["entity_id"]):
        # Get a friendly name if available
        friendly_name = entity.get("attributes", {}).get("friendly_name", entity["entity_id"])
        result += f"- **{entity['entity_id']}**: {entity['state']}"
        if friendly_name != entity["entity_id"]:
...
async def list_automations() -> list[dict[str, Any]]:
    ...
    try:
        ...
        # Type narrowing: at this point automations must be a list
        if not isinstance(automations, list):
            logger.warning("Error getting automations: Unexpected response format")
            return []
        ...
        logger.error(f"Error in list_automations: {e!s}")
        return []
...
@mcp.prompt()
def create_automation(trigger_type: str, entity_id: str | None = None) -> list[dict[str, str]]:
    """
...
@mcp.prompt()
def debug_automation(automation_id: str) -> list[dict[str, str]]:
    """
...
@mcp.prompt()
def troubleshoot_entity(entity_id: str) -> list[dict[str, str]]:
    """
...
@mcp.prompt()
def routine_optimizer() -> list[dict[str, str]]:
    """
...
@mcp.prompt()
def automation_health_check() -> list[dict[str, str]]:
    """
...
@mcp.prompt()
def entity_naming_consistency() -> list[dict[str, str]]:
    """
...
@mcp.prompt()
def dashboard_layout_generator() -> list[dict[str, str]]:
    """
    ...
    try:
        ...
        if history_data and isinstance(history_data, list):
            for state_list in history_data:
                # The API returns lists of state objects that we need to flatten
                states.extend(cast("list[dict[str, Any]]", state_list))
        ...
        logger.error(f"Error processing history for {entity_id}: {e!s}")
        return {
            "entity_id": entity_id,
            "error": f"Error processing history: {e!s}",
            "states": [],
            "count": 0,
        }
...
async def get_error_log(max_chars: int = 80000, tail_lines: int | None = None) -> dict[str, Any]:
    """
    ...
    """
    logger.info(
        f"Getting Home Assistant error log (max_chars={max_chars}, tail_lines={tail_lines})"
    )
